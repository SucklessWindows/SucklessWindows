# Credit: https://github.com/Atlas-OS/Atlas/blob/main/src/local-build.ps1
# Modify somethings to make it works with SW playbook, remmeber to enable the `Unblock` option in file properties if you download this script from Internet.
...
if (Get-Command '7z' -EA 0) {
	...
	throw "This script requires 7-Zip to be installed to continue. (https://7-zip.org/)"
}
...
while ($true) { Get-Content -Wait -LiteralPath $a -EA 0 | Write-Output; Start-Sleep 1 }
	...
	$startYmlPath = Seperator "Configuration\sw\start.yml"
